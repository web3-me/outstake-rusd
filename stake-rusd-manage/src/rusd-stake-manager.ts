...
export function handleExtendLockTime(event: ExtendLockTimeEvent): void {
  ...
  let stakeEntity = StakeRUSD.load(event.params.positionId.toString());
  if (stakeEntity !== null) {
    stakeEntity.deadline = event.params.newDeadLine;
    stakeEntity.save();
  }
...
export function handleStakeRUSD(event: StakeRUSDEvent): void {
  let entity = new StakeRUSD(event.params.positionId.toString());
  entity.positionId = event.params.positionId;
